...
CREATE OR REPLACE ROLE HOL_DE_ROLE;
GRANT ROLE HOL_DE_ROLE TO ROLE SYSADMIN;
GRANT ROLE HOL_DE_ROLE TO USER IDENTIFIER($MY_USER);

GRANT EXECUTE TASK ON ACCOUNT TO ROLE HOL_DE_ROLE;
GRANT MONITOR EXECUTION ON ACCOUNT TO ROLE HOL_DE_ROLE;
GRANT IMPORTED PRIVILEGES ON DATABASE SNOWFLAKE TO ROLE HOL_DE_ROLE;

-- Databases
CREATE OR REPLACE DATABASE HOL_DE_DB;
GRANT OWNERSHIP ON DATABASE HOL_DE_DB TO ROLE HOL_DE_ROLE;

-- Warehouses
CREATE OR REPLACE WAREHOUSE HOL_DE_WH WAREHOUSE_SIZE = XSMALL, AUTO_SUSPEND = 5, AUTO_RESUME= TRUE;
GRANT OWNERSHIP ON WAREHOUSE HOL_DE_WH TO ROLE HOL_DE_ROLE;
